...
void setup()  {
  ...
  for (int i = 0; i < scrambled +1; i++) { 
    ...
    //println(binaryToDec("00001011"));
    //bit_scrambled = binaryString(scrambledArray);
    //bit_solved = binaryString(solutionArray);
    //printArray(scrambledArray);
    //printArray(solutionArray);
    //println(bit_scrambled.length());
    sendScrambled = hamming(binaryString(scrambledArray));
    sendSolution = hamming(binaryString(solutionArray));
    //sendScrambled = hamming(bit_scrambled);
    //sendSolution = hamming(bit_solved);
    //println(sendScrambled.length());
    //println("");
    //println(sendSolution);
...
void draw() {
  ...
  /*while(port.available() > 0){  // printing data from arduino
      String recived = port.readString();
