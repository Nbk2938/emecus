...
void keyPressed() {                    // for user interface
  ...
  if(stopTiping == false){ //user input recognition
   switch(key)     //specific keys rappresent different moves
   {
     ...
     sequence.add(f); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     case 'F': 
     sequenceInToNubers.add(9);
     sequence.add(F); currentMove = sequence.get(counter); currentMove.start(); currentMove.update();
     //showSequence();
     break;
     case 'b': 
     sequenceInToNubers.add(11);
     sequence.add(b); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     case 'B': 
     sequenceInToNubers.add(10);
     sequence.add(B); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     case 'u': 
     sequenceInToNubers.add(2);
     sequence.add(u); currentMove = sequence.get(counter); currentMove.start(); currentMove.update();
     //showSequence();
     break;
     case 'U': 
     sequenceInToNubers.add(3);
     sequence.add(U); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     case 'd': 
     sequenceInToNubers.add(1);
    ...
    //showSequence();
     ...
     sequence.add(l); currentMove = sequence.get(counter); currentMove.start(); currentMove.update();
     //showSequence();
     break;
     case 'L': 
     sequenceInToNubers.add(6);
     sequence.add(L); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     case 'r': 
     sequenceInToNubers.add(4);
     sequence.add(r); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     case 'R': 
     sequenceInToNubers.add(5);
     sequence.add(R); currentMove = sequence.get(counter); currentMove.start(); currentMove.update(); 
     //showSequence();
     break;
     ...
     case 'i':  // x                                implementazione dei movimenti di asse  
   ...
   /*  if (key == '\n' ) { //se viene schiaccato 'enter' salva stringa e la resetta a 0 
       saved = typing; 
       System.out.println(saved);
       typing = 0; 
     } else if ((key > 47) && ( key < 58)){ // se viene schiacciato chiave (bottone tastiera) salva temporaneamente e se schiacciata un'altra aggiungi questa a 10 volte la prima.
      typing = 10 * typing + Character.getNumericValue(key); 
     }
    
    switch(saved) { //just for single algorithms
       //----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                                                  //tuti i casi possibili e algoritmi F2L (da 1 a 41)
       case 1:                                                   
       char F2L_1[] = {(2),(4),(3),(5)};
       for(int i : F2L_1){
         Move q = allMoves[i];
       ...
       //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                                                                      //tuti i casi possibili e algoritmi OLL (da 42 a 98)       
       case 42:                                                                       
       ...
       //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                                                              //tuti i casi possibili e algoritmi PLL (da 99 a 119)
       case 99:                                                                       
